...
The patch applied to `2.3.4` relates to the `lein repl` task: in addition to displaying a human readable info about the nREPL server's port & host, it appends a nREPL URL.
...
This greatly eases the opening of a Console View after having launched a headless lein repl via kbd:[Alt+L L] ( `your-project $ lein repl :headless` )
...
==== Missing source paths are not signaled as errors anymore

...
(defn- folding-range
  ...
  (let [open-token-end (lu/end-offset open-token-loc)
        close-token-start (lu/start-offset close-token-loc)]
    (trace :editor/text (str "computing folding for '" (lu/loc-text open-token-loc) "' -> offset end at " open-token-end
                             " and '" (lu/loc-text close-token-loc) "' -> offset start at " close-token-start))
    ; TODO - AR handle custom positions instances (see jdt's JavaElementPosition)
    (if (> close-token-start open-token-end)
      (Position. open-token-end (- close-token-start open-token-end))
      nil)))
...
(defn compute-folding!
  ...
  input IClojureEditor. The UI part is executed using asyncExec."

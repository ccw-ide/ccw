...
- Once discussion starts on the pull request, stop rewriting its history. Instead, add commits while the discussion unfolds explicitely, as it will be easier for everyone involved to check what has been modified. Working this way might require a rebase at the end of the discussion to put the changes back where they belong (if small) or some rewording if more conspicuous.

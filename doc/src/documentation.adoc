...
- Open the Java perspective (unnecessary for Counterclockwise Standalone product users)
...
- Run the project: 
** Click on the editor pane containing the code in `src/myproject/core.clj` and then select `Load file in REPL` from the `Clojure` menu. This sends the whole file's code to the REPL (and also takes care of starting a REPL for the project if none is currently started).
